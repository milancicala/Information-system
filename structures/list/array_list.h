...
namespace structures
{
	...
	{
		return (*array_)[index];		//Dereferencia musi byt
	}
	...
	inline void ArrayList<T>::add(const T & data)
	{
		if (size_ == array_->size()) 
			enlarge();
		(*array_)[size_++] = data;	//inkrementuje az po volani funkcie
	}
	...
	inline T ArrayList<T>::removeAt(const int index)
	{
		...
		size_--;
		return result;	//Result je kópia, takze moze byt
	}
	...
	{
		const ArrayListIterator& otherALI = dynamic_cast<const ArrayListIterator&>(other);
		return otherALI.position_ != position_ || otherALI.arrayList_ != arrayList_;
	}
